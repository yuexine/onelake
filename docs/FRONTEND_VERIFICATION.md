...
| §8.2.3 采集任务向导（4 步：选源→映射→CDC→调度） | `SyncTaskWizard.tsx`，真实数据源/schema/table/columns 驱动，试跑接 dry-run，接口失败显式错误/重试，无样例字段兜底 | ✅ |
| §8.2.4 任务详情 + 运行历史 + 实时日志 | `SyncTaskDetail.tsx`，触发/暂停/试跑/运行日志/取消运行接真实接口 | ✅ |
...
| §6.2 数据源创建数据面配置 | `DatasourceList` 新建抽屉支持 Airbyte source definition 加载与 workspace/source/destination 元信息保存 | ✅ |
