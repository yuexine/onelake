...
| Trino | trinodb/trino:latest | Iceberg 交互式 SQL 引擎 | 8080 |
| Airbyte | airbyte/airbyte (abctl) | 批/增量数据采集（300+ 连接器） | 8000 |
...
```
1) make up          # 起数据面（postgres/redis/minio/trino/keycloak/...）
2) make seed        # 初始化 Keycloak realm + MinIO bucket + Hive metastore
3) make migrate     # Flyway 在 8 个 schema 建表
4) make backend     # 本地跑 onelake-app（bootstrap 模块）
5) make frontend    # 起 web-console (vite dev server)
```
...
```bash
docker compose logs -f trino airbyte dagster   # 跟踪数据面日志
...
| Flyway 校验失败 | schema 缺失或历史脚本被改 | 确认 8 个 schema 已建；勿改历史脚本，用新版本号 |
...
- [ ]  `docker compose up -d` 全部组件健康，控制台可访问（MinIO/Trino/Airbyte/Dagster/OpenMetadata/Superset/Keycloak）
- [ ]  Keycloak 建好 `onelake` realm + 角色（DE/Admin/Consumer/Sec/Ops），`onelake-app` 客户端可签发 JWT
- [ ]  `make migrate`：Flyway 在 8 个 schema 建表成功，`audit_log` 等核心表存在
