...
export function NotificationCenter() {
  ...
  return (
    <Drawer
      ...
      extra={
        <Space>
          <Button size="small" icon={<CheckOutlined />} onClick={markAllRead}>全部已读</Button>
        ...
        items={CATEGORIES.map((c) => ({
          ...
          children: (
            <List
              ...
              renderItem={(n) => (
                <List.Item
                  actions={[<a key="view" onClick={() => { markRead(n.id); if (n.link) { navigate(n.link); setNotifyOpen(false); } }}>查看</a>]}
                >
                  <List.Item.Meta
